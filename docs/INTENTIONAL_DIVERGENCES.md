...
| 4 | **Dynamics** | Advances envelopes over zero-padded tail frames | Advances envelopes strictly over `valid_frames` | Preserves true decay state at stream end |
...
* **`cdsp` Enhancement**: In [`Pipeline/pipeline_transfer.c`](../src/Pipeline/pipeline_transfer.c), `cdsp` preserves mute flags across reloads.
...
### 1.4 Partial-Chunk Dynamics Envelope Progression
* **Upstream Behavior**: Upstream advances compressor, noise gate, and lookahead limiter envelopes across the full buffer capacity (`chunk_size`), including zero-padded tail frames on the stream's final partial chunk.
* **`cdsp` Enhancement**: In [`Processors/compressor_processor.c`](../src/Processors/compressor_processor.c), [`Processors/noise_gate_processor.c`](../src/Processors/noise_gate_processor.c), and [`Filters/lookahead_limiter.c`](../src/Filters/lookahead_limiter.c), `cdsp` advances envelope detection loops strictly across `valid_frames`.
* **Why `cdsp` Is Better**: Processing trailing zero padding causes envelope detectors to artificially decay/release into silence at stream termination. Restricting progression to valid frames preserves the true signal level and dynamics state.
...
* **Non-finite Sample Sanitization**: In [`Audio/sample_conversion.h`](../src/Audio/sample_conversion.h) and [`Processors/race_processor.c`](../src/Processors/race_processor.c), `cdsp` sanitizes NaN/Inf samples on format conversion and RACE feedback loops, preventing runaway positive feedback from destroying hardware.
* **Defensive Parameter Validation**: In [`Processors/compressor_processor.c`](../src/Processors/compressor_processor.c), [`Processors/noise_gate_processor.c`](../src/Processors/noise_gate_processor.c), [`Filters/clipper.c`](../src/Filters/clipper.c), and [`Filters/delay.c`](../src/Filters/delay.c), `cdsp` strictly rejects `NaN`, `Inf`, and out-of-range parameters (e.g. non-finite thresholds, clipper limits that underflow to zero, delays > 1e8 samples) during validation to prevent internal NaN propagation and mathematical singularities.
