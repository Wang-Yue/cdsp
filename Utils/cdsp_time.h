 ...
 * - Under macOS/Darwin: Uses `clock_gettime_nsec_np(CLOCK_UPTIME_RAW)` or `mach_absolute_time()`.
 * - Under Linux/POSIX: Uses `clock_gettime(CLOCK_MONOTONIC, &ts)` converted to nanoseconds.
 ...
 * 
 * Cross-platform helper unifying `Sleep` (Windows) and `nanosleep` / `usleep` (POSIX/macOS).
 * 
 ...
 * 
 * Cross-platform helper unifying sub-millisecond sleep implementation across OSes.
 * 
