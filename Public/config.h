...
/**
 * @brief Get the currently loaded configuration file path.
 * @param engine Pointer to the engine.
 * @return The path string, or NULL if none is loaded (do not free).
 */
const char* cdsp_get_config_path(const dsp_engine_t* engine);

/**
 * @brief Set the path to the configuration file (e.g. to be loaded upon reload).
 * @param engine Pointer to the engine.
 ...
 *
 * The output string is allocated dynamically. The caller must free it when done.
 *
 * @param engine Pointer to the engine.
 ...
 * @param out_err Pointer to write backend error information if application fails.
 * @return true on success, false on failure.
 */
...
/**
 * @brief Parse, configure, and start the engine using a config file on disk, with optional overrides.
 *
 * This utility abstracts the file reading, override application, and engine configuration sequence.
 *
 * @param engine Pointer to the engine.
 * @param path Path to the configuration file (YAML/JSON).
 * @param samplerate_override Override target sample rate (set to <= 0 to ignore).
 * @param channels_override Override capture channels (set to <= 0 to ignore).
 * @param format_override Override capture format string (set to NULL to ignore).
 * @param extra_samples_override Override capture extra samples (set to < 0 to ignore).
 * @param out_err Pointer to write backend error information if application fails.
 * @return true on success, false on failure.
 */
 ...
 * @param engine Pointer to the engine.
 * @return Allocated description string, or NULL if not present. Caller must free it.
 */
char* cdsp_get_config_description(const dsp_engine_t* engine);

/**
 * @brief Read a single value from the active configuration using a JSON Pointer path (RFC 6901).
 * @param engine Pointer to the engine.
 * @param json_ptr JSON Pointer path.
 * @return Allocated JSON string representation of the value, or NULL if path not found. Caller must free it.
 */
char* cdsp_get_config_value(const dsp_engine_t* engine, const char* json_ptr);

/**
 * @brief Set a single value in the active configuration using a JSON Pointer path.
 *
 ...
 * @param out_err Pointer to write backend error information if configuration fails.
 * @return true on success, false on failure.
 */
...
/**
 * @brief Parse and fill defaults for a JSON configuration string without applying it.
 *
 * @param json_str Inputs JSON configuration string.
 * @param out_result Output allocated JSON string with default values filled (or error description).
...
/**
 * @brief Parse and fill defaults for a YAML configuration string without applying it.
 *
 * @param yaml_str Inputs YAML configuration string.
 * @param out_result Output allocated YAML string with default values filled (or error description).
 * @param is_error Out boolean set to true if parsing/validation failed.
 * @return true on success, false on failure.
 */
...
/**
 * @brief Parse and fill defaults for a configuration file on disk without applying it.
 *
 * @param path Path to the configuration file.
 * @param out_result Output allocated configuration string (YAML format) with default values filled (or error description).
 * @param is_error Out boolean set to true if parsing/validation failed.
 * @return true on success, false on failure.
 */
