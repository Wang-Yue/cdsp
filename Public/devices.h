 ...
 *
 * The output array out_devices is allocated dynamically. The caller must free it when done.
 *
 ...
 * @param out_devices Output pointer to the allocated array of device info structures.
 ...
 *
 * The returned descriptor is allocated dynamically. The caller must free it by calling cdsp_free_device_capabilities.
 *
 ...
 * @param out_err Output pointer to write detailed device error if the query fails.
 * @return true on success, false on failure.
 */
