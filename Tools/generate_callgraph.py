...
def extract_calls_detailed(func_name, body):
    ...
    for line in lines:
        ...
        if re.search(r'if\s*\([^)]*(?:err|rerr|perr|failed|STOP_REASON|EOF|!ok|!got_data|!success|!core|!loop|!state|!chunk|reached_eos|!engine_shared_state_enqueue_garbage_pipeline)[^)]*\)', stripped):
            in_error_block = True
