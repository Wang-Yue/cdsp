...
dsp_session_get_processing_params(dsp_session_t *core) {
  ...
  if (!req &&
    ...
    if (last_capture_time > 0) {
      ...
      if (elapsed > timeout_sec) {
        if (engine_shared_state_get_state(core->shared) !=
        ...
        if (core->processing_params) {
          ...
          processing_parameters_set_measured_capture_rate(
              core->processing_params, 0.0);
...
dsp_session_get_stop_reason(const dsp_session_t *core) {
  if (core && core->shared) {
...
processing_stop_reason_t
dsp_session_stop_and_free(dsp_session_t *core,
