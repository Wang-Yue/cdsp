...
// Ref: docs/engine_state_management.md - Section 1.3: Controller Level
...
static void engine_on_chunk_captured_callback(void *ctx,
  ...
  if (buf && chunk)
    audio_history_buffer_append(buf, chunk);
}
...
static bool dsp_engine_set_config_struct_locked(dsp_engine_impl_t *impl,
  ...
  // Initialization Flow Step 1 Configuration Change Decision Tree: If devices
  // match, perform non-blocking pipeline hot-reload via
  // dsp_session_reload_config. If devices differ, fall back to full session
  ...
  // Ref: docs/engine_state_management.md - Section 1.7.1: Lifecycle & Ownership
...
static bool dsp_engine_set_config_json(void *ctx, const char *json_str,
  ...
  // Ref: docs/engine_state_management.md - Section 3.1: Startup &
  // Initialization Flow Step 1: Set config_in_progress to true. Status queries
  // check this atomic flag to return STARTING immediately without blocking on
  // the state_mutex.
...
static void dsp_engine_stop(void *ctx) {
  ...
  // Teardown Step 3: Controller teardown stops backend devices, joins
  // terminated threads, and cleans up session resources under the controller
  // lock.
  pthread_mutex_lock(&impl->state_mutex);
  if (impl->session.active) {
    dsp_session_collect_garbage(impl->session.active);
...
static void dsp_engine_set_fader_volume(void *ctx, fader_t fader, float db,
                                        bool instant) {
  if (!ctx || fader < 0 || fader >= FADER_COUNT)
    return;
  dsp_engine_impl_t *impl = (dsp_engine_impl_t *)ctx;
  pthread_mutex_lock(&impl->state_mutex);
...
static bool dsp_engine_get_samples(void *ctx, bool is_capture, size_t n_frames,
  ...
  size_t n = n_frames;
  if (n > AUDIO_HISTORY_BUFFER_CAPACITY)
    n = AUDIO_HISTORY_BUFFER_CAPACITY;
...
static void dsp_engine_set_state_file_path(void *ctx, const char *path) {
  dsp_engine_impl_t *impl = (dsp_engine_impl_t *)ctx;
  if (impl)
    engine_state_manager_set_state_file(impl->state_mgr, path);
}
...
static void dsp_engine_set_config_file_path(void *ctx, const char *path) {
  dsp_engine_impl_t *impl = (dsp_engine_impl_t *)ctx;
  if (impl)
    engine_state_manager_set_config_path(impl->state_mgr, path);
}
...
static void dsp_engine_free_impl(void *ctx) {
  ...
  if (impl->buffers.spectrum)
    spectrum_analyzer_free(impl->buffers.spectrum);
  if (impl->buffers.capture)
    audio_history_buffer_free(impl->buffers.capture);
  if (impl->buffers.playback)
    audio_history_buffer_free(impl->buffers.playback);
  if (impl->state_mgr)
    engine_state_manager_free(impl->state_mgr);
  pthread_mutex_destroy(&impl->state_mutex);
  if (impl->config.active_json)
    free(impl->config.active_json);
  if (impl->config.previous_json)
    free(impl->config.previous_json);
  free(impl);
}
...
// Flow Step 1 (Unified Main-Thread Watchdog) & Section 1.7.1 (Garbage
// Collection via dsp_session_collect_garbage)
