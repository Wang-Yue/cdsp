...
// (is_shutdown) & Section 3.6: Immediate Abort Teardown Step 2: Unblocking
// worker threads via queue shutdown.
...
// Ref: docs/engine_state_management.md - Section 3.2: Steady-State Audio Loops
// (SPSC Queue + OS Semaphore) & Section 3.5: Graceful EOF Teardown (Queue Drain
// Steps 2-3).
void *audio_sync_queue_dequeue_blocking(audio_sync_queue_t *queue) {
  if (!queue || !queue->queue || !queue->semaphore)
    return NULL;
