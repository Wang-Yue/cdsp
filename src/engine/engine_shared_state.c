...
spsc_queue_t *
engine_shared_state_get_captured_queue(engine_shared_state_t *state) {
  return state ? audio_sync_queue_get_spsc_queue(state->captured_queue) : NULL;
}

spsc_queue_t *
engine_shared_state_get_processed_queue(engine_shared_state_t *state) {
...
double
engine_shared_state_get_resampler_ratio(const engine_shared_state_t *state) {
...
double
engine_shared_state_get_capture_pitch(const engine_shared_state_t *state) {
  return state
...
engine_shared_state_get_stop_reason(const engine_shared_state_t *state) {
...
engine_shared_state_create(size_t captured_queue_depth,
...
audio_chunk_t *
engine_shared_state_dequeue_captured_blocking(engine_shared_state_t *state) {
  if (!state)
    return NULL;
...
engine_shared_state_dequeue_processed_blocking(engine_shared_state_t *state) {
  if (!state)
    return NULL;
  audio_chunk_t *chunk = (audio_chunk_t *)audio_sync_queue_dequeue_blocking(
...
processing_state_t
engine_shared_state_get_state(const engine_shared_state_t *state) {
  if (!state)
...
uint64_t
engine_shared_state_get_last_capture_time(const engine_shared_state_t *state) {
