...
mutable_waveform_t audio_buffers_get_channel(const audio_buffers_t *buffers,
                                             size_t ch) {
  if (!buffers || ch >= buffers->channels)
    return NULL;
...
audio_buffers_t *audio_buffers_copy_from(const double *const *waveforms,
                                         ...
                                         size_t channels) {
  if (channels == 0 || !waveforms || !channel_lengths)
    return NULL;
  ...
  size_t max_cap = 0;
  for (size_t ch = 0; ch < channels; ch++) {
    if (channel_lengths[ch] > max_cap)
      max_cap = channel_lengths[ch];
  }
  if (max_cap == 0)
    max_cap = 1;
...
void audio_buffers_free(audio_buffers_t *buffers) {
  ...
  if (buffers->channel_buffers)
    free(buffers->channel_buffers);
  free(buffers);
}

void audio_buffers_zero(audio_buffers_t *buffers) {
  if (!buffers || !buffers->storage)
    return;
