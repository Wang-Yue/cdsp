...
static void loudness_filter_free(void *instance) {
  ...
  if (!filter)
    return;
  if (filter->low_shelf_filter)
    g_biquad_vtable.free(filter->low_shelf_filter);
...
static int loudness_config_validate(const filter_config_t *config,
  ...
  (void)sample_rate;
  if (!config || config->type != FILTER_TYPE_LOUDNESS)
    return -1;
...
static void loudness_filter_process(void *instance, mutable_waveform_t waveform,
  ...
  // bypass processing.
  if (!filter->is_processing_active)
    return;
...
const filter_vtable_t g_loudness_vtable = {.validate = loudness_config_validate,
                                           ...
                                           .transfer_state =
                                               loudness_filter_transfer_state,
