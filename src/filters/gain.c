...
static void *gain_filter_create(const char *name, const filter_config_t *config,
  ...
  if (!config || config->type != FILTER_TYPE_GAIN)
    return NULL;
  const gain_config_t *params = &config->parameters.gain;
  if (gain_config_validate(config, 0, err) != 0)
    return NULL;
 ...
 */
static void gain_filter_process(void *instance, mutable_waveform_t waveform,
                                size_t count) {
...
const filter_vtable_t g_gain_vtable = {.validate = gain_config_validate,
