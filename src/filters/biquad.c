...
static void *biquad_filter_create(const char *name,
  ...
  if (config) {
    ...
    if (biquad_config_validate(config, sample_rate, err) != 0)
      return NULL;
  }
...
static void biquad_filter_process(void *instance, mutable_waveform_t waveform,
  ...
  if (!filter)
    return;
  if (!waveform || count == 0) {
    if (fpclassify(filter->z1) == FP_SUBNORMAL)
      filter->z1 = 0.0;
    if (fpclassify(filter->z2) == FP_SUBNORMAL)
      filter->z2 = 0.0;
    return;
  }
...
static void biquad_choose_split(size_t channels, size_t depth,
  ...
  if (stages < 1)
    stages = 1;
  if (stages > BIQUAD_MAX_DEPTH)
    stages = BIQUAD_MAX_DEPTH;

  size_t group = channels;
  if (group < 1)
    group = 1;
  if (group > BIQUAD_MAX_CHANNELS)
    group = BIQUAD_MAX_CHANNELS;
...
void biquad_process_mono_cascade(biquad_filter_t **stages, size_t num_stages,
  ...
  if (!stages || num_stages == 0 || !waveform || n_frames == 0)
    return;
 ...
 * A^2 = (2 * (1 + a2) * (s1^2 + s2^2) - 4 * a1 * s1 * s2) / ((1 + a2)^2 -
 * a1^2). Denominator factors as (1 + a2 - a1) * (1 + a2 + a1), the stability
 * triangle.
 *
...
const filter_vtable_t g_biquad_vtable = {.validate = biquad_config_validate,
                                         .create = biquad_filter_create,
                                         .process = biquad_filter_process,
                                         .transfer_state =
                                             biquad_filter_transfer_state,
