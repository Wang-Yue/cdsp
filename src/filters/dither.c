...
static double noise_shaper_process(noise_shaper_t *shaper, double scaled,
                                   double dither) {
  if (!shaper || shaper->filter_count == 0)
    return round(scaled + dither);
  double filt_buf = 0.0;
...
static noise_shaper_t *noise_shaper_create_for_type(dither_type_t type) {
  ...
  case DITHER_TYPE_SHIBATA_LOW_48: {
    const double c[] = {
        ...
        0.03214230760931969,   1.2108691862522392e-6,   0.0,
...
static double sample_dither(dither_filter_t *filter) {
  if (filter->type == DITHER_TYPE_NONE)
    return 0.0;
...
static int dither_config_validate(const filter_config_t *config,
  ...
  (void)sample_rate;
  if (!config || config->type != FILTER_TYPE_DITHER)
    return -1;
...
static void *dither_filter_create(const char *name,
  ...
  if (!config || config->type != FILTER_TYPE_DITHER)
    return NULL;
  const dither_config_t *params = &config->parameters.dither;
  if (dither_config_validate(config, 0, err) != 0)
    return NULL;
...
static void noise_shaper_transfer_state(noise_shaper_t *dest,
  ...
  if (!dest || !src || dest == src)
    return;
  if (!dest->buffer || !src->buffer || dest->filter_count == 0)
    return;
  if (dest->filter_count != src->filter_count)
    return;
...
const filter_vtable_t g_dither_vtable = {.validate = dither_config_validate,
                                         .create = dither_filter_create,
                                         .process = dither_filter_process,
                                         .transfer_state =
                                             dither_filter_transfer_state,
