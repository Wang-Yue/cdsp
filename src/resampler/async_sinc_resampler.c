...
static sinc_interpolation_type_t
sinc_interpolation_type_from_string(const char *str) {
  if (!str)
    return SINC_INTERPOLATION_QUADRATIC;
  if (strcmp(str, "Nearest") == 0)
    return SINC_INTERPOLATION_NEAREST;
  if (strcmp(str, "Linear") == 0)
    return SINC_INTERPOLATION_LINEAR;
  if (strcmp(str, "Quadratic") == 0)
    return SINC_INTERPOLATION_QUADRATIC;
  if (strcmp(str, "Cubic") == 0)
    return SINC_INTERPOLATION_CUBIC;
...
calculate_input_size(size_t chunk_size, double resample_ratio,
                     double target_ratio, double last_index,
                     size_t interpolator_len, fixed_async_t fixed) {
...
static inline size_t
calculate_output_size(size_t chunk_size, double resample_ratio,
                      double target_ratio, double last_index,
                      size_t interpolator_len, fixed_async_t fixed) {
...
static void
async_sinc_resampler_update_lengths(async_sinc_resampler_t *resampler) {
...
static void run_cubic(async_sinc_resampler_t *resampler, size_t output_frames,
  ...
  if (get_active_channel_count(resampler, output) >= 2) {
    ...
    for (size_t frame = 0; frame < output_frames; frame++) {
      ...
      if (pts[1].idx < min_idx)
        min_idx = pts[1].idx;
      if (pts[2].idx < min_idx)
        min_idx = pts[2].idx;
      if (pts[3].idx < min_idx)
        min_idx = pts[3].idx;
    ...
    for (size_t ch = 0; ch < resampler->channels; ch++) {
      ...
      for (size_t frame = 0; frame < output_frames; frame++) {
        ...
        // Horner form with FMA: a0 + x*(a1 + x*(a2 + x*a3))
  ...
  if (get_active_channel_count(resampler, output) > 2) {
    ...
    for (size_t ch = 0; ch < resampler->channels; ch++) {
      ...
      for (size_t frame = 0; frame < output_frames; frame++) {
        ...
        double a0 = 2.0 * p0;
        // Horner form with FMA: 0.5 * (a0 + x*(a1 + x*a2))
        double x = frac_offset;
...
static resampler_error_t
async_sinc_resampler_process(void *impl, const audio_chunk_t *input,
                             audio_chunk_t *output) {
  async_sinc_resampler_t *resampler = (async_sinc_resampler_t *)impl;
 ...
 */
static int
async_sinc_resampler_config_validate(const resampler_config_t *config,
                                     config_error_t *err) {
  ...
  if (has_free) {
    if (!config->has_sinc_len || !config->has_oversampling_factor ||
      ...
      config_error_set(
          err, CONFIG_ERR_VALIDATION,
          "AsyncSinc: incomplete free parameter set; sinc_len, "
    ...
    if (config->oversampling_factor < min_oversampling) {
      config_error_set(err, CONFIG_ERR_VALIDATION,
                       "oversampling_factor must be at least %d for %s "
                       "interpolation, got %d",
                       min_oversampling, config->interpolation,
  ...
  if (config->has_interpolation) {
    ...
    if (interp == SINC_INTERPOLATION_LAST) {
      config_error_set(err, CONFIG_ERR_VALIDATION,
...
double
async_sinc_resampler_get_cutoff(const async_sinc_resampler_t *resampler) {
