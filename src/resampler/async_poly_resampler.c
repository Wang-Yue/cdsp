...
static poly_interpolation_t poly_interpolation_from_string(const char *str) {
  ...
  if (strcmp(str, "Linear") == 0)
    return POLY_INTERPOLATION_LINEAR;
  if (strcmp(str, "Cubic") == 0)
    return POLY_INTERPOLATION_CUBIC;
  if (strcmp(str, "Quintic") == 0)
    return POLY_INTERPOLATION_QUINTIC;
  if (strcmp(str, "Septic") == 0)
    return POLY_INTERPOLATION_SEPTIC;
...
static void
async_poly_resampler_update_lengths(async_poly_resampler_t *resampler) {
...
async_poly_resampler_process(void *impl, const audio_chunk_t *input,
                             audio_chunk_t *output) {
  async_poly_resampler_t *resampler = (async_poly_resampler_t *)impl;
 ...
 */
static int
async_poly_resampler_config_validate(const resampler_config_t *config,
                                     config_error_t *err) {
...
double
async_poly_resampler_get_cutoff(const async_poly_resampler_t *resampler) {
  (void)resampler;
  return -1.0;
}
