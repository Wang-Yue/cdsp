...
static void synchronous_resampler_free(void *impl) {
  ...
  if (resampler->working_spec)
    cdsp_aligned_free(resampler->working_spec);
  free(resampler);
}
...
synchronous_resampler_process(void *impl, const audio_chunk_t *input,
  ...
  for (size_t s = 0; s < resampler->num_subchunks; s++) {
    ...
    if (sub_valid > resampler->sub_fft_in)
      sub_valid = resampler->sub_fft_in;
 ...
 */
static int
synchronous_resampler_config_validate(const resampler_config_t *config,
                                      config_error_t *err) {
  (void)err;
  if (!config || config->type != RESAMPLER_TYPE_SYNCHRONOUS)
    return -1;
  return 0;
}
