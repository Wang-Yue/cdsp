...
static const processor_vtable_t *
processor_vtable_from_type(processor_type_t type) {
  switch (type) {
...
processor_impl_type_from_config(processor_type_t type) {
  switch (type) {
  case PROCESSOR_TYPE_COMPRESSOR:
...
dsp_processor_t *dsp_processor_create(const char *name,
  ...
  if (processor_config_validate(config, sample_rate, err) != 0)
    return NULL;
