...
static void race_processor_free(void *impl) {
  ...
  if (processor->delay_a)
    g_delay_vtable.free(processor->delay_a);
  if (processor->delay_b)
    g_delay_vtable.free(processor->delay_b);
  if (processor->gain)
    g_gain_vtable.free(processor->gain);
  free(processor);
}
...
static void *race_processor_create(const char *name,
  ...
  if (race_config_validate(config, sample_rate, err) != 0)
    return NULL;
  if (sample_rate <= 0) {
    config_error_set(err, CONFIG_ERR_INVALID_PROCESSOR,
  ...
  processor->gain = (gain_filter_t *)g_gain_vtable.create("race-Gain", &gcfg, 0,
                                                          0, NULL, err);
...
static void race_processor_process(void *impl, audio_chunk_t *chunk) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (!isfinite(processor->feedback_a))
      processor->feedback_a = 0.0;
    if (!isfinite(processor->feedback_b))
      processor->feedback_b = 0.0;
...
const processor_vtable_t g_race_vtable = {.validate = race_config_validate,
                                          .create = race_processor_create,
                                          .process = race_processor_process,
                                          .get_name = race_processor_get_name,
                                          .transfer_state =
                                              race_processor_transfer_state,
