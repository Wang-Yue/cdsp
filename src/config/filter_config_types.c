...
fader_t fader_from_string(const char *str) {
  ...
  if (strcmp(str, "Aux1") == 0)
    return FADER_AUX1;
  if (strcmp(str, "Aux2") == 0)
    return FADER_AUX2;
  if (strcmp(str, "Aux3") == 0)
    return FADER_AUX3;
  if (strcmp(str, "Aux4") == 0)
    return FADER_AUX4;
  return FADER_NONE;
}
...
filter_type_t filter_type_from_string(const char *str) {
  ...
  if (strcmp(str, "Gain") == 0)
    return FILTER_TYPE_GAIN;
  if (strcmp(str, "Volume") == 0)
    return FILTER_TYPE_VOLUME;
  if (strcmp(str, "Loudness") == 0)
    return FILTER_TYPE_LOUDNESS;
  if (strcmp(str, "Biquad") == 0)
    return FILTER_TYPE_BIQUAD;
  if (strcmp(str, "Conv") == 0)
    return FILTER_TYPE_CONV;
  if (strcmp(str, "Delay") == 0)
    return FILTER_TYPE_DELAY;
  if (strcmp(str, "BiquadCombo") == 0)
    return FILTER_TYPE_BIQUAD_COMBO;
  if (strcmp(str, "DiffEq") == 0)
    return FILTER_TYPE_DIFF_EQ;
  if (strcmp(str, "Dither") == 0)
    return FILTER_TYPE_DITHER;
  if (strcmp(str, "Clipper") == 0)
    return FILTER_TYPE_CLIPPER;
