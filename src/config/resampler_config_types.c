...
resampler_type_t resampler_type_from_string(const char *str) {
  ...
  if (strcmp(str, "Synchronous") == 0)
    return RESAMPLER_TYPE_SYNCHRONOUS;
  if (strcmp(str, "AsyncSinc") == 0)
    return RESAMPLER_TYPE_ASYNC_SINC;
  if (strcmp(str, "AsyncPoly") == 0)
    return RESAMPLER_TYPE_ASYNC_POLY;
  if (strcmp(str, "Slip") == 0)
    return RESAMPLER_TYPE_SLIP;
  return RESAMPLER_TYPE_SYNCHRONOUS;
}
...
resampler_profile_t resampler_profile_from_string(const char *str) {
  ...
  if (strcmp(str, "VeryFast") == 0)
    return RESAMPLER_PROFILE_VERY_FAST;
  if (strcmp(str, "Fast") == 0)
    return RESAMPLER_PROFILE_FAST;
  if (strcmp(str, "Balanced") == 0)
    return RESAMPLER_PROFILE_BALANCED;
  if (strcmp(str, "Accurate") == 0)
    return RESAMPLER_PROFILE_ACCURATE;
  return RESAMPLER_PROFILE_BALANCED;
}
