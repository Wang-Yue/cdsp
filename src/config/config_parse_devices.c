  ...
  case RESAMPLER_TYPE_ASYNC_SINC: {
    ...
    if (cJSON_GetObjectItemCaseSensitive(res_obj, "profile")) {
      ...
      if (parse_enum_required(res_obj, "profile", sinc_profiles,
                              "AsyncSinc resampler", &enum_scratch, err) != 0) {
        return -1;
      }
    } else {
    ...
    if (parse_labels_array_strict(cap_labels_node, &cap->labels,
      ...
      if (err) {
        config_error_set(err, CONFIG_ERR_INVALID_DEVICE,
                         "Invalid 'labels' array in capture device: elements "
...
static int parse_playback(const cJSON *play_obj, devices_config_t *devices,
  ...
  if (play_labels_node) {
    if (parse_labels_array_strict(play_labels_node, &play->labels,
      ...
      if (err) {
        config_error_set(err, CONFIG_ERR_INVALID_DEVICE,
                         "Invalid 'labels' array in playback device: elements "
                         "must be strings or null");
      }
      return -1;
    }
  ...
  case AUDIO_BACKEND_TYPE_PIPEWIRE:
    ...
    snprintf(final_play->cfg.pipewire.node_name,
             sizeof(final_play->cfg.pipewire.node_name), "%s", temp.node_name);
