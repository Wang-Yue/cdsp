  ...
  cJSON_ArrayForEach(filter_child, filters_obj) {
    ...
    }
    {
      ...
      case FILTER_TYPE_VOLUME: {
        static const char *const allowed[] = {"ramp_time_ms", "limit", "fader",
                                              NULL};
        if (validate_unknown_fields(params, allowed, "Volume filter parameters",
                                    err) != 0)
          return -1;
      ...
      case FILTER_TYPE_LOUDNESS: {
        ...
        if (require_json_fields(params, req_loudness,
          ...
          return -1;
        lp->has_reference_level =
            parse_json_double(params, "reference_level", &lp->reference_level);
        lp->has_high_boost =
        ...
        lp->has_low_freq = parse_json_double(params, "low_freq", &lp->low_freq);
      ...
      case FILTER_TYPE_BIQUAD: {
        static const char *const allowed[] = {
            "type",     "freq",   "gain",        "q",        "bandwidth",
            "slope",    "a1",     "a2",          "b0",       "b1",
            "b2",       "freq_z", "freq_p",      "q_p",      "normalize_at_dc",
            "freq_act", "q_act",  "freq_target", "q_target", NULL};
        if (validate_unknown_fields(params, allowed, "Biquad filter parameters",
                                    err) != 0)
          return -1;
        ...
        if (bp->type == BIQUAD_TYPE_FREE) {
          ...
          if (validate_unknown_fields(params, allowed_fo,
                                      "Biquad FO filter parameters", err) != 0)
            return -1;
        } else if (bp->type == BIQUAD_TYPE_HIGHSHELF_FO ||
                   bp->type == BIQUAD_TYPE_LOWSHELF_FO) {
          static const char *const allowed_shelf_fo[] = {"type", "freq", "gain",
                                                         NULL};
          ...
          static const char *const allowed_pass[] = {"type", "freq", "q", NULL};
        ...
        }
        {
          ...
          static const char *const req_notch[] = {"freq_p", "freq_z", "q_p",
                                                  NULL};
          static const char *const req_lt[] = {"freq_act", "q_act",
                                               "freq_target", "q_target", NULL};
          ...
          if (required &&
              require_json_fields(params, required, "Biquad filter parameters",
                                  variant, err) != 0)
            return -1;
          if (width &&
              require_json_any_field(params, width, "Biquad filter parameters",
                                     variant, err) != 0)
            return -1;
        }
      ...
      case FILTER_TYPE_DELAY: {
        static const char *const allowed[] = {"delay", "delay_unit",
                                              "subsample", NULL};
        if (validate_unknown_fields(params, allowed, "Delay filter parameters",
                                    err) != 0)
          return -1;
      ...
      case FILTER_TYPE_CONV: {
        ...
        if (parse_enum_required(params, "type", conv_types,
                                "Conv filter parameters", &conv_type, err) != 0)
          return -1;
        ...
        if (cp->type == CONV_TYPE_RAW) {
          ...
          if (validate_unknown_fields(params, allowed_dum,
                                      "Conv Dummy filter parameters", err) != 0)
            return -1;
        }
        ...
        if (parse_json_size_t_strict(params, "skip_bytes_lines", "Conv filter",
                                     &cval, &cpresent, err) != 0)
          return -1;
        if (cpresent)
          cp->skip_bytes_lines = cval;

        if (parse_json_size_t_strict(params, "read_bytes_lines", "Conv filter",
                                     &cval, &cpresent, err) != 0)
          return -1;
        if (cpresent)
          cp->read_bytes_lines = cval;
        break;
      }
      case FILTER_TYPE_BIQUAD_COMBO: {
        static const char *const allowed[] = {"type",     "freq",  "order",
                                              ...
                                              "freq_max", "gains", NULL};
        if (validate_unknown_fields(params, allowed,
                                    "BiquadCombo filter parameters", err) != 0)
          return -1;
        ...
        if (bcp->type == BIQUAD_COMBO_TYPE_BUTTERWORTH_HIGHPASS ||
          ...
          if (validate_unknown_fields(params, allowed_tilt,
                                      "BiquadCombo Tilt parameters", err) != 0)
            return -1;
          ...
          static const char *const allowed_geq[] = {"type", "gains", "freq_min",
                                                    "freq_max", NULL};
      ...
      case FILTER_TYPE_DIFF_EQ: {
        ...
        if (validate_unknown_fields(params, allowed, "DiffEq filter parameters",
                                    err) != 0)
          return -1;
      ...
      case FILTER_TYPE_DITHER: {
        static const char *const allowed[] = {"type", "bits", "amplitude",
                                              NULL};
        if (validate_unknown_fields(params, allowed, "Dither filter parameters",
                                    err) != 0)
          return -1;
        ...
        dp->type = (dither_type_t)dither_type;
        {
          ...
          if (require_json_fields(params, required, "Dither filter parameters",
                                  variant, err) != 0)
            return -1;
        }
        ...
        if (parse_json_size_t_strict(params, "bits", "Dither filter parameters",
                                     &bits_val, &bits_present, err) != 0)
          return -1;
        if (bits_present)
          dp->bits = (int)bits_val;
        dp->has_amplitude =
            parse_json_double(params, "amplitude", &dp->amplitude);
        break;
      }
      case FILTER_TYPE_CLIPPER: {
        static const char *const allowed[] = {"clip_limit", "soft_clip", NULL};
        if (validate_unknown_fields(params, allowed,
      ...
      case FILTER_TYPE_LOOKAHEAD_LIMITER: {
        static const char *const allowed[] = {
            "limit", "attack", "release", "attack_unit", "release_unit", NULL};
        if (validate_unknown_fields(params, allowed,
        ...
        if (parse_json_str(params, "attack_unit", a_unit_buf,
          ...
          if (strcmp(a_unit_buf, "us") == 0)
            ...
            config_error_set(err, CONFIG_ERR_INVALID_FILTER,
                             "Filter '%s': invalid attack_unit '%s'", nf->name,
                             a_unit_buf);
            return -1;
          }
        } else {
        ...
        if (parse_json_str(params, "release_unit", r_unit_buf,
          ...
          if (strcmp(r_unit_buf, "us") == 0)
            ...
            config_error_set(err, CONFIG_ERR_INVALID_FILTER,
                             "Filter '%s': invalid release_unit '%s'", nf->name,
                             r_unit_buf);
            return -1;
          }
        } else {
...
int config_parse_processors(const cJSON *processors_obj, dsp_config_t *config,
  ...
  config->processors = (named_processor_config_t *)calloc(
      size, sizeof(named_processor_config_t));
  ...
  cJSON_ArrayForEach(proc_child, processors_obj) {
    ...
    case PROCESSOR_TYPE_COMPRESSOR: {
      static const char *const allowed[] = {
          ...
          NULL};
      if (validate_unknown_fields(params, allowed,
                                  "Compressor processor parameters", err) != 0)
        return -1;
      ...
      if (parse_json_str(params, "attack_unit", a_unit_buf,
        ...
        if (strcmp(a_unit_buf, "us") == 0)
          ...
          config_error_set(err, CONFIG_ERR_INVALID_PROCESSOR,
                           "Processor '%s': invalid attack_unit '%s'", np->name,
                           a_unit_buf);
          return -1;
        }
      } else {
    ...
    case PROCESSOR_TYPE_NOISE_GATE: {
      ...
      if (require_json_fields(params, req_gate,
                              "NoiseGate processor parameters", NULL, err) != 0)
        return -1;
    ...
    case PROCESSOR_TYPE_RACE: {
      ...
      if (validate_unknown_fields(params, allowed, "RACE processor parameters",
                                  err) != 0)
        return -1;
      ...
      if (parse_json_size_t_strict(params, "channels",
          ...
          parse_json_size_t_strict(params, "channel_a",
                                   "RACE processor parameters", &rp->channel_a,
                                   NULL, err) != 0 ||
          parse_json_size_t_strict(params, "channel_b",
                                   "RACE processor parameters", &rp->channel_b,
                                   NULL, err) != 0)
        return -1;
      ...
      if (parse_json_str(params, "delay_unit", unit_buf, sizeof(unit_buf))) {
        if (strcmp(unit_buf, "us") == 0)
          ...
          config_error_set(err, CONFIG_ERR_INVALID_PROCESSOR,
                           "Processor '%s': invalid delay_unit '%s'", np->name,
                           unit_buf);
          return -1;
        }
    ...
    case PROCESSOR_TYPE_LOOKAHEAD_LIMITER: {
      ...
      if (validate_unknown_fields(params, allowed,
        ...
        return -1;
      static const char *const req_lim[] = {
          "channels", "attack", "attack_unit", "release", "release_unit", NULL};
      if (require_json_fields(params, req_lim,
