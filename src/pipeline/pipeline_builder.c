...
static biquad_step_t *
build_biquad_step(const char *const *names, size_t names_count,
                  const dsp_config_t *config, int rate, const size_t *channels,
                  size_t channels_count, size_t chunk_channels,
                  config_error_t *err) {
...
static bool
resolve_filter_step_channels(const pipeline_step_config_t *step,
                             size_t current_channels, size_t **out_channels,
                             size_t *out_count, size_t *out_single_ch,
...
pipeline_t *pipeline_create(const dsp_config_t *config,
  ...
  if (pipeline_config_validate(config, err) != 0)
    return NULL;
