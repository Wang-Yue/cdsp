...
int core_audio_device_streams(AudioDeviceID device_id, core_audio_scope_t scope,
  ...
  if (count > max_streams && out_streams)
    count = max_streams;
...
void core_audio_device_set_pitch(AudioDeviceID device_id, double pitch) {
  AudioObjectPropertyAddress addr = {.mSelector = kAudioDevicePropertyStereoPan,
...
bool rate_change_watcher_get_pending_change(rate_change_watcher_t *watcher,
  ...
  if (fabs(rate - watcher->expected_rate) < 0.5)
    return false;
  if (out_rate)
    *out_rate = rate;
  return true;
 ...
 */
static const char *
format_string_for_asbd_local(const AudioStreamBasicDescription *asbd) {
...
void core_audio_device_release_hog_mode(AudioDeviceID device_id) {
  ...
  pid_t current_hog_pid = -1;
  uint32_t hog_size = sizeof(pid_t);
  AudioObjectPropertyAddress addr = {.mSelector = kAudioDevicePropertyHogMode,
                                     .mScope = kAudioObjectPropertyScopeGlobal,
                                     .mElement =
                                         kAudioObjectPropertyElementMain};
...
void core_audio_device_stop_and_destroy_ioproc(
  ...
  if (device_id == 0 || io_proc_id == NULL)
    return;
