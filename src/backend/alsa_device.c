...
snd_hctl_elem_t *alsa_find_elem(snd_hctl_t *hctl, snd_ctl_elem_iface_t iface,
  ...
  if (device >= 0)
    snd_ctl_elem_id_set_device(id, (unsigned int)device);
  if (subdevice >= 0)
  ...
  if (elem) {
    ...
    if (out_numid)
      *out_numid = snd_ctl_elem_id_get_numid(found_id);
...
bool alsa_elem_read_as_int(snd_hctl_elem_t *elem, long *out_val) {
  ...
  if (snd_hctl_elem_read(elem, val) >= 0) {
    if (out_val)
      *out_val = snd_ctl_elem_value_get_integer(val, 0);
    return true;
  }
  return false;
}
...
bool alsa_elem_read_as_bool(snd_hctl_elem_t *elem, bool *out_val) {
  ...
  if (snd_hctl_elem_read(elem, val) >= 0) {
    if (out_val)
      *out_val = (snd_ctl_elem_value_get_boolean(val, 0) != 0);
    return true;
  }
  return false;
}
...
bool alsa_elem_read_volume_in_db(snd_ctl_t *ctl, snd_hctl_elem_t *elem,
  ...
  if (!ctl || !elem)
    return false;
  long intval = 0;
  if (!alsa_elem_read_as_int(elem, &intval))
    return false;
...
calculate_buffer_size_alt(size_t chunksize, double resampling_ratio,
                          snd_pcm_uframes_t min_period) {
  if (resampling_ratio <= 0.0)
    resampling_ratio = 1.0;
  double frames_needed = 3.0 * (double)chunksize / resampling_ratio;
