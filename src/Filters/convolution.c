...
static void convolution_filter_free(void* instance) {
  ...
  if (filter->hist_f) cdsp_aligned_free(filter->hist_f);
  if (filter->temp_buf) cdsp_aligned_free(filter->temp_buf);
  if (filter->fft) real_fft_free(filter->fft);
...
static int convolution_config_validate(const filter_config_t* config,
  ...
  switch (params->type) {
    ...
    case CONV_TYPE_WAV: {
      ...
      char err_msg[512] = {0};
      size_t count = 0;
      double* probe = wav_read_channel_samples(
          params->filename, params->channel, &count, err_msg, sizeof(err_msg));
      if (!probe || count == 0) {
        free(probe);
        if (err_msg[0] != '\0') {
