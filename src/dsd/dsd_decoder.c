  ...
  for (size_t t = 0; t < frames; t++) {
    ...
    if (mode == DSD_MODE_NATIVE) {
      ...
      if (dsd_bit_depth == 8) {
        ...
        uint32_t u32 = pcm_sample_encode_dsd_u32(raw);
...
void dsd_decoder_free(dsd_decoder_t *decoder) {
  if (!decoder)
    return;
  if (decoder->channel_states)
    cdsp_aligned_free(decoder->channel_states);
  if (decoder->ctables)
    free(decoder->ctables);
  free(decoder);
}
