...
static int notify_capture_rate(const char *card, int device, int subdevice,
                               unsigned int rate) {
  char ctl_name[64];
  if (card &&
      (strncmp(card, "hw:", 3) == 0 || strncmp(card, "sysdefault:", 11) == 0)) {
  ...
  snd_ctl_elem_id_set_subdevice(id,
                                subdevice >= 0 ? (unsigned int)subdevice : 0);
  ...
  } else if (snd_ctl_elem_info_get_max(info) < (long)rate ||
             snd_ctl_elem_info_get_max(info) < 1536000) {
...
static int rn_poll_descriptors(snd_pcm_ioplug_t *io, struct pollfd *pfd,
                               unsigned int space) {
  rate_notify_ioplug_t *rec = (rate_notify_ioplug_t *)io;
  if (rec->slave)
...
static int rn_poll_revents(snd_pcm_ioplug_t *io, struct pollfd *pfd,
...
static int set_slave_hw_params(snd_pcm_t *slave, snd_pcm_hw_params_t *params) {
  ...
  if (snd_pcm_hw_params_get_buffer_size(params, &buffer_size) >= 0 &&
      buffer_size > 0) {
    snd_pcm_hw_params_set_buffer_size_near(slave, sp, &buffer_size);
  }
  if (snd_pcm_hw_params_get_period_size(params, &period_size, 0) >= 0 &&
      period_size > 0) {
...
static bool is_capture_active_at_rate(const char *card, long device,
...
static int rn_hw_params(snd_pcm_ioplug_t *io, snd_pcm_hw_params_t *params) {
  ...
  bool format_changed = (rec->last_format != SND_PCM_FORMAT_UNKNOWN &&
                         format != rec->last_format);

  // 1. If format changed even without a rate change, signal cdsp to restart by
  // toggling Capture Rate
  ...
  if (rate > 0) {
    notify_capture_rate(rec->ctl_card, rec->ctl_device, rec->ctl_subdevice,
                        rate);
  }
  ...
  // This locks the loopback cable to the player's format & rate in the ALSA
  // kernel.
  int err = -EBUSY;
  ...
  // When cdsp probes formats with format: null, the ALSA kernel will only offer
  // the player's format!
  for (int i = 0; i < rec->retry_count; i++) {
...
CDSP_PLUGIN_EXPORT SND_PCM_PLUGIN_DEFINE_FUNC(rate_notify) {
  ...
  // 2. Sample Rates: continuous range 8 kHz to 768 kHz (matching snd-aloop &
  // cdsp)
  snd_pcm_ioplug_set_param_minmax(&rec->io, SND_PCM_IOPLUG_HW_RATE, 8000,
                                  768000);

  // 3. Formats: complete list of PCM, Float, and DSD formats supported by ALSA
  // & cdsp
  ...
  snd_pcm_ioplug_set_param_minmax(&rec->io, SND_PCM_IOPLUG_HW_BUFFER_BYTES,
                                  1024, 2 * 1024 * 1024);
  snd_pcm_ioplug_set_param_minmax(&rec->io, SND_PCM_IOPLUG_HW_PERIOD_BYTES, 256,
                                  512 * 1024);
