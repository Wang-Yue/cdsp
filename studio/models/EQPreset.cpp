...
EQBandType stringToEQBandType(const std::string& str) {
    ...
    if (str == "LowpassFO")
        return EQBandType::LowpassFO;
    if (str == "HighpassFO")
        return EQBandType::HighpassFO;
    if (str == "LowshelfFO")
        return EQBandType::LowshelfFO;
    if (str == "HighshelfFO")
        return EQBandType::HighshelfFO;
    if (str == "Notch")
        return EQBandType::Notch;
    if (str == "Bandpass")
        return EQBandType::Bandpass;
    if (str == "Allpass")
        return EQBandType::Allpass;
    if (str == "AllpassFO")
        return EQBandType::AllpassFO;
    if (str == "Free")
        return EQBandType::Free;
    if (str == "GeneralNotch")
        return EQBandType::GeneralNotch;
    if (str == "LinkwitzTransform")
        return EQBandType::LinkwitzTransform;
...
std::string eqBandTypeToShortName(EQBandType type) {
    ...
    case EQBandType::Allpass:
        return "AP";
    default:
        return eqBandTypeToString(type);
    }
}
...
std::optional<BiquadCoefficients> EQBand::coefficients(int sampleRate) const {
    ...
    default:
        params.freq = freq;
        if (eqBandTypeHasGain(type))
            params.gain = gain;
        if (eqBandTypeHasQ(type))
            params.q = q;
        break;
    }
...
QJsonObject EQBand::toJson() const {
    ...
    obj["useSlope"] = useSlope;
    obj["useBandwidth"] = useBandwidth;
    obj["freqAct"] = freqAct;
    obj["qAct"] = qAct;
    obj["freqTarget"] = freqTarget;
    obj["qTarget"] = qTarget;
    return obj;
}
...
EQBand EQBand::fromJson(const QJsonObject& json) {
    EQBand b;
    if (json.contains("id"))
        b.id = QUuid::fromString(json["id"].toString());
    if (json.contains("type"))
        b.type = stringToEQBandType(json["type"].toString().toStdString());
    ...
    if (json.contains("isEnabled"))
        b.isEnabled = json["isEnabled"].toBool();
    if (json.contains("b0"))
    ...
    if (json.contains("useSlope"))
        b.useSlope = json["useSlope"].toBool();
    if (json.contains("useBandwidth"))
        b.useBandwidth = json["useBandwidth"].toBool();
    if (json.contains("freqAct"))
        b.freqAct = json["freqAct"].toDouble();
    if (json.contains("qAct"))
        b.qAct = json["qAct"].toDouble();
    if (json.contains("freqTarget"))
        b.freqTarget = json["freqTarget"].toDouble();
    if (json.contains("qTarget"))
        b.qTarget = json["qTarget"].toDouble();
    return b;
}
...
std::optional<EQPreset> EQPreset::fromCSV(const std::string& text, const std::string& presetName) {
    ...
    while (std::getline(ss, line)) {
        ...
        if (lower.find("graphiceq:") == 0) {
            std::string content = trimmed.substr(10);
            std::replace(content.begin(), content.end(), ';', ' ');
            std::replace(content.begin(), content.end(), ',', ' ');
            std::stringstream gss(content);
            double freqVal, gainVal;
            while (gss >> freqVal >> gainVal) {
                EQBand band(EQBandType::Peaking, freqVal, gainVal, 1.41, true);
                parsedBands.push_back(band);
            }
            continue;
        }

        bool isFilterLine = (lower.find("filter") == 0);
        std::string content = trimmed;
        if (isFilterLine) {
            size_t pos = trimmed.find(':');
            content = (pos != std::string::npos) ? trimmed.substr(pos + 1) : trimmed.substr(6);
        }
        ...
        if (!words.empty()) {
            ...
            if (idx < words.size()) {
                std::string typeWord = words[idx];
                std::string upperType = typeWord;
                std::transform(upperType.begin(), upperType.end(), upperType.begin(), ::toupper);

                EQBandType type = shortNameToEQBandType(typeWord);
                bool validFilter = isFilterLine || upperType == "PK" || upperType == "LS" || upperType == "HS" ||
                                   upperType == "LP" || upperType == "HP" || upperType == "NO" || upperType == "BP" ||
                                   upperType == "AP" || upperType == "LSC" || upperType == "HSC" ||
                                   upperType == "LPC" || upperType == "HPC" || upperType == "PEAKING" ||
                                   upperType == "LOWSHELF" || upperType == "HIGHSHELF" || upperType == "LOWPASS" ||
                                   upperType == "HIGHPASS" || upperType == "NOTCH" || upperType == "BANDPASS" ||
                                   upperType == "ALLPASS";
...
EQPreset EQPreset::fromJson(const QJsonObject& json) {
    EQPreset p;
    if (json.contains("id"))
        p.id = QUuid::fromString(json["id"].toString());
    if (json.contains("name"))
        p.name = json["name"].toString().toStdString();
    if (json.contains("preampGain"))
        p.preampGain = json["preampGain"].toDouble();
