...
StageBuildResult StageBuilders::buildStage(
    ...
    case StageType::Balance: {
        ...
        for (int ch = 0; ch < channelCount; ++ch) {
            MixerMapping m;
            m.dest = ch;
            if (ch == 0) m.sources.push_back(MixerSource{0, leftDB, false});
            else if (ch == 1) m.sources.push_back(MixerSource{1, rightDB, false});
    ...
    case StageType::Width: {
        ...
        for (int ch = 0; ch < channelCount; ++ch) {
            MixerMapping m;
            m.dest = ch;
            if (ch == 0) m.sources = makeSources(0, 1, ll, lr);
            else if (ch == 1) m.sources = makeSources(0, 1, lr, ll);
            else m.sources.push_back(MixerSource{ch, 0.0, false});
            mc.mapping.push_back(m);
        }
    ...
    case StageType::MSProc: {
        ...
        for (int ch = 0; ch < channelCount; ++ch) {
            ...
            m.dest = ch;
            if (ch == 0) {
                m.sources.push_back(MixerSource{0, -6.02, false});
                m.sources.push_back(MixerSource{1, -6.02, false});
            } else if (ch == 1) {
                m.sources.push_back(MixerSource{0, -6.02, false});
                m.sources.push_back(MixerSource{1, -6.02, true});
            } else {
    ...
    case StageType::PhaseInvert: {
        ...
        if (stage.invertLeft) step.channels.push_back(0);
        if (stage.invertRight && channelCount > 1) step.channels.push_back(1);
    ...
    case StageType::Crossfeed: {
        ...
        std::string fGainKey = baseKey + "_lo_gain";

        res.filters[fHiKey] = fHi;
        res.filters[fLoKey] = fLo;
        res.filters[fGainKey] = fGain;

        PipelineStep step1, step2, step3;
        step1.type = PipelineStepType::Filter; step1.names.push_back(fHiKey);
        for (int ch = 0; ch < channelCount; ++ch) step1.channels.push_back(ch);
        step2.type = PipelineStepType::Filter; step2.names.push_back(fLoKey);
        for (int ch = 0; ch < channelCount; ++ch) step2.channels.push_back(ch);
        step3.type = PipelineStepType::Filter; step3.names.push_back(fGainKey);
        for (int ch = 0; ch < channelCount; ++ch) step3.channels.push_back(ch);
    ...
    case StageType::SplitWidth: {
        ...
        PipelineStep stepLp, stepHp;
        stepLp.type = PipelineStepType::Filter; stepLp.names.push_back(fLpKey);
        for (int ch = 0; ch < channelCount; ++ch) stepLp.channels.push_back(ch);
        stepHp.type = PipelineStepType::Filter; stepHp.names.push_back(fHpKey);
        for (int ch = 0; ch < channelCount; ++ch) stepHp.channels.push_back(ch);
    ...
    case StageType::GraphicEQ: {
        ...
        step.type = PipelineStepType::Filter;
        step.names.push_back(fKey);
        for (int ch = 0; ch < channelCount; ++ch) step.channels.push_back(ch);
        res.steps.push_back(step);
        break;
    }
