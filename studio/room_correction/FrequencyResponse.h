...
public:
    ...
    FrequencyResponse(const std::vector<double>& real, const std::vector<double>& imag, int sampleRate = 48000, int fftSize = 4096);
    ...
    static std::vector<std::pair<double, FrequencyResponse>> stft(
        const ImpulseResponse& ir,
        int sliceCount = 30,
        double maxTimeSeconds = 0.4,
