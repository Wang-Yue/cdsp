...
std::vector<double> PEQAutoFit::smoothLogOctave(
    const std::vector<double>& values,
    const std::vector<double>& frequencies,
    double midOctaves,
    double trebleOctaves,
    double transitionLowHz,
    double transitionHighHz
) {
    if (values.size() != frequencies.size() || values.empty()) return values;
...
static void accumulateBandResponse(const BiquadParameters& band, const std::vector<double>& frequencies, int sampleRate, std::vector<double>& residual) {
    auto coeffs = BiquadCoefficients::compute(band, sampleRate);
    if (!coeffs.has_value()) return;
    for (size_t i = 0; i < frequencies.size(); ++i) {
        residual[i] += coeffs.value().gainDB(frequencies[i], sampleRate);
    }
}

static std::optional<BiquadParameters> seedPeak(const std::vector<double>& residual, const std::vector<double>& frequencies, const PEQAutoFitOptions& options) {
    int bestIdx = -1;
    double bestAbs = 0.0;
    ...
    if (bestIdx < 0 || bestAbs < options.convergenceDB) return std::nullopt;

    double peak = residual[bestIdx];
    double halfTarget = std::abs(peak) * 0.5;
    double sign = peak >= 0 ? 1.0 : -1.0;
    int l = bestIdx;
    while (l > 0 && residual[l - 1] * sign >= halfTarget) l--;
    int r = bestIdx;
    while (r < static_cast<int>(residual.size()) - 1 && residual[r + 1] * sign >= halfTarget) r++;
...
static std::optional<BiquadParameters> seedShelf(
    BiquadType type,
    std::pair<double, double> edgeBand,
    double cornerHz,
    ...
    if (std::abs(median) < options.convergenceDB) return std::nullopt;
...
static double cost(const BiquadParameters& band, const std::vector<double>& rwb, const std::vector<double>& frequencies, int sampleRate) {
    auto coeffs = BiquadCoefficients::compute(band, sampleRate);
    if (!coeffs.has_value()) return 1e18;
    double total = 0.0;
    for (size_t i = 0; i < frequencies.size(); ++i) {
...
static double goldenSectionSearch(double lo, double hi, double tolerance, bool logSpace, std::function<double(double)> f) {
    double phi = (std::sqrt(5.0) - 1.0) / 2.0;
    double a = logSpace ? std::log10(lo) : lo;
    double b = logSpace ? std::log10(hi) : hi;
    if (a >= b) return logSpace ? std::pow(10.0, a) : a;
...
static BiquadParameters optimizeBand(
    const BiquadParameters& band,
    const std::vector<double>& rwb,
    const std::vector<double>& frequencies,
    int sampleRate,
    const PEQAutoFitOptions& options
) {
    ...
    for (int iter = 0; iter < 2; ++iter) {
        ...
        curr.gain = goldenSectionSearch(-options.maxGainDB, options.maxGainDB, 0.02, false, [&](double g) {
            BiquadParameters b = curr; b.gain = g;
        ...
        if (curr.type != BiquadType::Lowshelf && curr.type != BiquadType::Highshelf) {
            ...
            if (hi > lo) {
                curr.freq = goldenSectionSearch(lo, hi, 0.001, true, [&](double freq) {
                    BiquadParameters b = curr; b.freq = freq;
                    return cost(b, rwb, frequencies, sampleRate);
                });
            }
        } else {
...
std::vector<BiquadParameters> PEQAutoFit::fit(
    ...
    size_t n = freqs.size();
    if (measuredMagDB.size() != n || n <= 4) return {};
    ...
    std::vector<double> baseResidual = smoothLogOctave(
        rawResidual, freqs,
        options.smoothingOctaves,
        options.trebleSmoothingOctaves,
        options.smoothingTransitionLow,
        options.smoothingTransitionHigh
    );
    ...
    if (options.addEndpointShelves && !suppressLowShelf) {
        auto lo = seedShelf(BiquadType::Lowshelf, {options.minFreqHz, options.lowShelfFreqHz}, options.lowShelfFreqHz, residual, freqs, options);
        if (lo.has_value()) {
            bands.push_back(lo.value());
            accumulateBandResponse(lo.value(), freqs, sampleRate, residual);
        }
        auto hi = seedShelf(BiquadType::Highshelf, {options.highShelfFreqHz, options.maxFreqHz}, options.highShelfFreqHz, residual, freqs, options);
        if (hi.has_value()) {
    ...
    for (int iter = 0; iter < options.refinementIterations; ++iter) {
        ...
        for (size_t i = 0; i < bands.size(); ++i) {
            ...
            for (size_t j = 0; j < bands.size(); ++j) {
                if (j != i) accumulateBandResponse(bands[j], freqs, sampleRate, rwb);
            }
    ...
    bands.erase(std::remove_if(bands.begin(), bands.end(), [&](const BiquadParameters& b) {
        return std::abs(b.gain.value_or(0.0)) < options.dropGainDB;
    }), bands.end());
