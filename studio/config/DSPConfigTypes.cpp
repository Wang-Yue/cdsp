...
void DSPConfiguration::validate() const {
    ...
    if (devices.targetLevel.has_value()) {
        ...
        if (target <= 0)
            throw std::runtime_error("Target level must be positive");
        int qlim = devices.queuelimit.value_or(4);
        int maxTarget = (2 + qlim) * devices.chunksize;
        if (target > maxTarget)
            throw std::runtime_error("target_level cannot be larger than max limit");
    }
    if (devices.queuelimit.has_value()) {
        int q = devices.queuelimit.value();
        if (q < 0 || q > 1000)
            throw std::runtime_error("Queue limit must be between 0 and 1000");
    }
    ...
    for (size_t i = 0; i < pipeline.size(); ++i) {
        ...
        if (step.bypassed.value_or(false))
            continue;
        ...
        if (step.type == PipelineStepType::Mixer) {
            ...
            if (names.empty() && step.name.has_value())
                names.push_back(step.name.value());
            for (const auto& n : names) {
                if (!filters.count(n)) {
    ...
    if (currentChannels != playbackChannels) {
        throw std::runtime_error("Pipeline outputs " + std::to_string(currentChannels) +
                                 " channel(s) but playback device expects " + std::to_string(playbackChannels));
    }
}
