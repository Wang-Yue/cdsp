...
void VectorScopeView::paintEvent(QPaintEvent* event) {
    ...
    // 1. Phosphor decay: fade existing buffer content slightly toward background
    {
        ...
        if (count > 0) {
            ...
            float rawCorr = (sumL2 > 1e-9 && sumR2 > 1e-9) ? static_cast<float>(sumLR / (std::sqrt(sumL2 * sumR2))) : 1.0f;
    ...
    // 4. Draw Phase Correlation Meter Bar (-1 to +1) (Bottom)
    {
        ...
        QColor corrColor;
        if (corrVal > 0.3f) corrColor = QColor(52, 199, 89);        // Green (Mono in-phase)
        else if (corrVal >= -0.3f) corrColor = QColor(255, 204, 0); // Yellow (Decorrelated)
        else corrColor = QColor(255, 59, 48);                       // Red (Out of phase)
