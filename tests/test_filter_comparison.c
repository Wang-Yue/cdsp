...
static bool run_harness_arr(const char *argv[], size_t argc) {
  ...
  if (!bin) {
    printf("⚠️ skipping: harness not found — build with `cargo build --release "
           "--manifest-path tests/rust_harnesses/Cargo.toml`\n");
    return false;
  }
  char cmd[4096];
#ifdef _WIN32
