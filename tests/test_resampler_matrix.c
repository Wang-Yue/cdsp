...
static bool check_rubato_available(void) {
  ...
  printf("⚠️ skipping: rubato harness not found — build with `cargo build "
         "--release --manifest-path tests/rust_harnesses/Cargo.toml`\n");
...
static double *run_rubato(const char *mode, int in_rate, int out_rate,
  ...
  remove(out_path);
  if (!write_raw_f64(input, input_count, in_path))
    return NULL;
...
static bool measure_swift_perf(int in_rate, int out_rate, resampler_impl_t impl,
  ...
  if (max_in_buf < base_cs * 4)
    max_in_buf = base_cs * 4;
...
static bool measure_rubato_perf(const char *mode, int in_rate, int out_rate,
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (p)
      ns_total = strtoull(p + 15, NULL, 10);
    p = strstr(line, "BENCH_OUT_FRAMES_PER_ITER=");
    if (p)
      out_frames_per_iter = (int)strtol(p + 26, NULL, 10);
    p = strstr(line, "BENCH_ITERS=");
    if (p)
      iters = (int)strtol(p + 12, NULL, 10);
  }
